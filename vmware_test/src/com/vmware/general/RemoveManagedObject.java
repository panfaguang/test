...
public class RemoveManagedObject {
   ...
   private static void getInputParameters(String[] args) {
      ...
      while (ai < args.length) {
         ...
         if (param.equalsIgnoreCase("--objtype") && !val.startsWith("--") &&
               !val.isEmpty()) {
            objecttype = val;
         } else if (param.equalsIgnoreCase("--objname") && !val.startsWith("--") &&
                      !val.isEmpty()) {
            objectname = val;
      ...
      if(objecttype == null || objectname == null) {
         throw new IllegalArgumentException("Expected --objtype and --objname argument.");
      }
   }
   ...
   private static boolean validateTheInput() {
      ...
      if (!(objecttype.equalsIgnoreCase("HostSystem")
            ...
            || objecttype.equalsIgnoreCase("ResourcePool")
            || objecttype.equalsIgnoreCase("Datacenter"))) {
   ...
   */
   ...
   private static void deleteManagedObjectReference() {
      try {
         ManagedObjectReference objmor = getEntityByName(objectname, objecttype);
         ...
         if (objmor != null) {
            ...
            System.out.println(" : Failed " + operation + " of " +
            objecttype + " : " + objectname);
         }
      } catch (SOAPFaultException sfe) {
         printSoapFaultException(sfe);
