...
public class UploadGuestFile {
   ...
   private static String guestFilePath;
   private static String localFilePath;
   private static String guestType;
   ...
   private static void printUsage() {
      System.out
            .println("This sample uploads a file from the client machine "+
                   "to a specified locaion inside the guest");
      System.out
      ...
      System.out
            .println("guestfilepath       [required]:Path of the file in the Guest OS");
      System.out
            .println("localfilepath       [required]:Local file path to Upload");
      System.out
            .println("guesttype           [required]:Type of the guest. (Windows or Posix)");
      System.out
              .println("overwrite           [optional]: Whether to overwrite the file in the guest.");
      System.out.println("<b>Command Line:To upload a file</b>");
      System.out
            .println("run.bat com.vmware.samples.guest.UploadGuestFile "
                  + "--url [webserviceurl]");
      System.out.println("--username [username] --password  [password] "
            + "--vmname [vmname]");
      System.out
            .println("--guestusername [guest user] --guestpassword [guest password]");
      System.out.println("--guestfilepath [path of the file inside the guest]");
      System.out.println("--localfilepath [local file path to upload]");
      System.out.println("--guesttype [windows or posix]");
  ...
  private static ManagedObjectReference getVmByVMname(String vmName) {
     ...
     try {
        ...
        List<ObjectContent> listobcont = vimPort.retrieveProperties(propCollector, listpfs);
   ...
   private static void uploadData(String urlString,
   ...
   public static void main(String[] args) {
      try {
         ...
         guestFilePath = optionsmap.get("guestfilepath");
         localFilePath = optionsmap.get("localfilepath");
         guestType = optionsmap.get("guesttype");
         ...
         if (guestType.equalsIgnoreCase("windows")) {
           GuestWindowsFileAttributes windowsFileAttributes = new GuestWindowsFileAttributes();
           guestFileAttributes = windowsFileAttributes;
         } else {
           GuestPosixFileAttributes posixFileAttributes = new GuestPosixFileAttributes();
           posixFileAttributes.setOwnerId(1);
           posixFileAttributes.setGroupId(1);
           posixFileAttributes.setPermissions((long) 0644);
           guestFileAttributes = posixFileAttributes;
         }
         ...
         File file = new File(optionsmap.get("localfilepath"));
         if (file == null) {
         ...
         String fileUploadUrl = vimPort.initiateFileTransferToGuest(fileManagerRef,
                                                                    vmMOR,
                                                                    auth,
                                                                    guestFilePath,
                                                                    guestFileAttributes,
                                                                    fileSize,
                                                                    optionsmap.containsKey("overwrite"));
   ...
   private static boolean verifyInputArguments() throws Exception {
      ...
      vinput.add(guestFilePath);
      vinput.add(localFilePath);
      vinput.add(guestType);
