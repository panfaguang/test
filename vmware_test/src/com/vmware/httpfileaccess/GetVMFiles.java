 ...
 *
 * <b>Parameters:</b> url [required] : url of the web service. username [required] : username for the authentication
 * password [required] : password for the authentication vmname [required] : Name of the virtual machine localpath
 * [required] : localpath to copy files
 *
 * <b>Command Line:</b> To get the virtual machine files on local disk run.bat com.vmware.httpfileaccess.GetVMFiles
 * --url [webserviceurl] --username [username] --password [password] --vmname [vmname] --localpath [localpath] </pre>
 */
public class GetVMFiles {
        ...
        if (args.length != 0) {
            if (args.length < 2) {
                ...
                if (param.equalsIgnoreCase("--help")) {
                    ...
                    throw new IllegalArgumentException("Expected --url," + " --username, --password arguments.");
                }
            }
        }
        int ai = 0;
    ...
    /*
     * This method check server type VC or ESX.getVMDFile is applicable only on ESXand not for VCreturn: boolean value
     * true for ESX
     */
    ...
    private static void getInputParameters(String[] args) {
        ...
        while (ai < args.length) {
            ...
            if (param.equalsIgnoreCase("--vmname") && !val.startsWith("--") && !val.isEmpty()) {
                vmName = val;
            }
            if (param.equalsIgnoreCase("--localpath") && !val.startsWith("--") && !val.isEmpty()) {
                localPath = val;
            }
            val = "";
            ai += 2;
    ...
    private static void getVM() throws IllegalArgumentException, Exception {
        ...
        if (vmRef != null) {
            ...
            if (vmDirectory[0] != null) {
                System.out.println("vmDirectory-0: " + vmDirectory[0] + " datacenter as : " + dataCenterName);
                ...
                String configurationDir = vmDirectory[0].substring(vmDirectory[0].indexOf("]") + 2,
                        vmDirectory[0].lastIndexOf("/"));
                boolean success = new File(localPath + "/" + configurationDir + "#vm#" + dataStoreName).mkdir();
                downloadDirectory(configurationDir, configurationDir + "#vm#" + dataStoreName, dataStoreName,
                        dataCenterName);
            ...
            if (vmDirectory[1] != null) {
                ...
                for (int i = 1; i < vmDirectory.length; i++) {
                    String dataStoreName = vmDirectory[i].substring(vmDirectory[i].indexOf("[") + 1,
                ...
                for (int i = 0; i < virtualDiskLocations.length; i++) {
                    if (virtualDiskLocations[i] != null) {
                        String dataStoreName = virtualDiskLocations[i].substring(
                                virtualDiskLocations[i].indexOf("[") + 1, virtualDiskLocations[i].lastIndexOf("]"));
                        String configurationDir = virtualDiskLocations[i].substring(
                                virtualDiskLocations[i].indexOf("]") + 2, virtualDiskLocations[i].lastIndexOf("/"));
                        if (!downloadedDir.containsKey(configurationDir + "#vm#" + dataStoreName)) {
                            boolean success = new File(localPath + "/" + configurationDir + "#vdisk#" + dataStoreName)
                                    .mkdir();
                            downloadDirectory(configurationDir, configurationDir + "#vdisk#" + dataStoreName,
    ...
    private static String[] getVmDirectory(ManagedObjectReference vmmor) throws Exception {
        String[] vmDir = new String[4];
        VirtualMachineConfigInfo vmConfigInfo = (VirtualMachineConfigInfo) getProperty(vmmor, "config");
    ...
    private static void getDiskSizeInKB(ManagedObjectReference vmMor) throws Exception {
        ...
        if (vmConfigInfo != null) {
            ...
            for (int i = 0; i < vDevice.length; i++) {
                if (vDevice[i].getClass().getCanonicalName().contains("com.vmware.vim25.VirtualDisk")) {
                    try {
    ...
    private static String[] getVDiskLocations(ManagedObjectReference vmmor) throws Exception {
        VirtualMachineConfigInfo vmConfigInfo = (VirtualMachineConfigInfo) getProperty(vmmor, "config");
        ...
        if (vmConfigInfo != null) {
            ...
            for (int i = 0; i < vDevice.length; i++) {
                if (vDevice[i].getClass().getCanonicalName().equalsIgnoreCase("com.vmware.vim25.VirtualDisk")) {
                    try {
                        long size = ((VirtualDisk) vDevice[i]).getCapacityInKB();
                        System.out.println("Disk size in kb: " + size);
                        VirtualDeviceFileBackingInfo backingInfo = (VirtualDeviceFileBackingInfo) vDevice[i]
                                .getBacking();
    ...
    private static void downloadDirectory(String directoryName, String localDirectory, String dataStoreName,
            String dataCenter) throws Exception {
        String serviceUrl = url;
        serviceUrl = serviceUrl.substring(0, serviceUrl.lastIndexOf("sdk") - 1);
        String httpUrl = serviceUrl + "/folder/" + directoryName + "?dcPath=" + dataCenter + "&dsName=" + dataStoreName;
        ...
        for (int i = 0; i < linkMap.length; i++) {
            ...
            String fileName = localDirectory + "/"
                    + linkMap[i].substring(linkMap[i].lastIndexOf("/"), linkMap[i].lastIndexOf("?"));
    ...
    private static ArrayList<String> getFileLinks(String xmlString) throws Exception {
    ...
    private static void getData(String urlString, String fileName) throws Exception {
        HttpURLConnection conn = null;
        URL urlSt = new URL(urlString);
        conn = (HttpURLConnection) urlSt.openConnection();
        conn.setDoInput(true);
    ...
    private static String getDataCenter(ManagedObjectReference vmmor) throws Exception {
    ...
    private static Object getProperty(ManagedObjectReference hostmor, String propertyname) throws Exception {
     ...
     */
    private static Object getDynamicProperty(ManagedObjectReference mor, String propertyName) throws Exception {
        ...
        if (objContent != null) {
            ...
            if (listdp != null) {
                ...
                if (dynamicPropertyName.indexOf("ArrayOf") != -1) {
                    ...
                    /*
                     * If object is ArrayOfXXX object, then get the invoking getXXX() on the object. For Ex:
     ...
     */
    private static ObjectContent[] getObjectProperties(ManagedObjectReference mobj, String[] properties)
            throws Exception {
        if (mobj == null) {
            return null;
        }
    ...
    public static void main(String[] args) {
        try {
            ...
            url = "https://192.168.2.41/sdk";
            userName = "root";
            password = "antrol&support";
            vmName = "win7";
            connect();
            getVM();
